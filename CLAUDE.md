...
2. Create the file directly under `quartz/content/<type-folder>/` with
   `publish: true`.
3. Commit and push to `main` (confirm first, since a push is outward-facing).
...
Never use em dashes, in content, code comments, commit messages or anything
else written here. Use a comma, colon, semicolon or a full stop instead.
