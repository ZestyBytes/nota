...
function topicLatest(id){return state.data.entries.filter(e=>e.topics?.includes(id)).map(e=>e.occurredAt||e.createdAt||"").sort().pop()||""}
function topics(){
  const sorts={items:"Most kept",name:"A to Z",recent:"Recent"};
  const list=Object.entries(state.data.topics).map(([id,t])=>({id,t,count:topicCount(id),latest:topicLatest(id)}));
  ...
  return `<section>${pageHead("Paths through the archive","Topics","Each topic has a quiet default appearance, or an optional view shaped around its material, without changing the underlying taxonomy.")}<div class="search-filters topic-sort">${Object.entries(sorts).map(([k,label])=>`<button class="filter ${state.topicSort===k?"active":""}" data-topicsort="${k}">${label}</button>`).join("")}</div><div class="topic-grid">${list.map(({id,t,count,latest})=>`<button class="topic-card" data-topic="${id}" style="--topic:${t.color};--soft:${t.soft}"><span class="topic-mark" aria-hidden="true">${esc(t.name[0])}</span><span class="topic-count">${count} ${count===1?"item":"items"}</span><h2>${esc(t.name)}</h2><p>${esc(t.description)}</p><span class="topic-foot">${t.mode?"Tailored view":latest?fmtDate(latest):"&nbsp;"}</span></button>`).join("")}</div></section>`;
}
...
function topicView(id){const t=topic(id),items=state.data.entries.filter(e=>e.topics?.includes(id)),books=state.data.books.filter(b=>b.topics?.includes(id));let body=`<div class="entry-list">${items.map(entryCard).join("")||(books.length?"":`<p class="empty">Nothing in this topic yet.</p>`)}</div>`;if(t.mode==="listen"){
