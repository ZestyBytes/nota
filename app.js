...
// Each fact carries its own mark, so the strip can be read at a glance from
// across the room rather than word by word. Light and water also carry a level,
// a filled mark out of three, worked out from what the note already says.
function plantLevel(kind,p){
  if(kind==="Light"){const t=String(p.light||"").toLowerCase();
    if(!t)return 0;
    if(/full sun|bright/.test(t))return 3;
    if(/medium|part/.test(t))return 2;
    if(/low|shade|dim/.test(t))return 1;
    return 2;}
  if(kind==="Water"){const d=p.waterEvery;
    if(!d)return 0;
    return d<=7?3:d<=12?2:1;}
  return 0;
}
function plantFacts(p){
  return [["Light","sun",p.light],["Water","drop",p.water||(p.waterEvery?`Every ${p.waterEvery} days`:"")],
    ["Feed","feed",p.feed],["Humidity","mist",p.humidity],["Soil","soil",p.soil],
    ["Position","home",p.position],["Pets","paw",p.pets]].filter(([,,v])=>v);
}
function levelMarks(n,total=3){
  return `<span class="plant-level" aria-hidden="true">${Array.from({length:total},(_,i)=>`<i class="${i<n?"on":""}"></i>`).join("")}</span>`;
}
function plantBody(e,shown){
  const p=e.plant,facts=plantFacts(p),due=waterDue(p);
  return `${facts.length?`<dl class="plant-facts">${facts.map(([k,ic,v])=>{const lvl=plantLevel(k,p);return `<div><span class="plant-fact-mark" aria-hidden="true">${icon(ic)}</span><dt>${k}</dt><dd>${esc(String(v))}</dd>${lvl?levelMarks(lvl):""}</div>`}).join("")}</dl>`:""}
...
function calendar(){
  ...
  return `<section class="calendar-page">
    <header class="calendar-toolbar"><div class="calendar-head"><button type="button" class="icon-button" data-month="-1" aria-label="Previous month">←</button><label class="calendar-title"><h2 id="calendar-heading">${first.toLocaleDateString("en-GB",{month:"long",year:"numeric"})}</h2><input type="month" data-calendar-month value="${monthKey}" min="1000-01" max="9999-12" aria-label="Jump to month and year"></label><button type="button" class="icon-button" data-month="1" aria-label="Next month">→</button></div>
    <div class="calendar-controls"><button class="calendar-today" data-jump="${todayKey}" type="button">Today</button><div class="calendar-modes" aria-label="Calendar view">${["month","agenda"].map(mode=>`<button class="${state.calendarMode===mode?"active":""}" aria-pressed="${state.calendarMode===mode}" data-calendar-mode="${mode}">${mode==="month"?"Month":"Agenda"}</button>`).join("")}</div></div></header>
...
function libraryBody(){
  ...
  else if(state.library==="reading")body=`<div class="book-grid">${state.data.books.map(b=>`<article class="book ${b.cover?"":"has-plate"}" data-book="${b.id}"><span class="acc-no">No. ${accNo(b.id)}</span>${b.cover?`<img data-cover="${esc(b.id)}" class="book-cover" src="${b.cover}" alt="" loading="lazy">`:coverPlate(b)}<div class="book-copy"><h3>${esc(b.title)}</h3><p>${esc(b.author)}</p><div class="book-links"><span>${(b.notes||[]).length} notes</span><span>${(b.quotes||[]).length} quotes</span></div><span class="status">${esc(b.status.replaceAll("-"," "))}${b.status==="reading"?` · ${b.progress}%`:""}</span><div class="progress"><i style="width:${b.progress}%"></i></div></div></article>`).join("")||`<p class="empty">Your library is empty.</p>`}</div>`;
...
function spacePreview(id,photo){
  if(id==="reading"){
    ...
    return covers.length?`<span class="space-reading-preview" aria-hidden="true">${covers.map(b=>`<img data-cover="${esc(b.id)}" src="${esc(b.cover)}" alt="">`).join("")}</span>`:"";
  }
...
// bar. Keeping the office plants has none of that, so it gets a dot per entry: the
...
const TOPIC_ROOMS={
  ...
  wedding:{label:"The wedding shelf",section:"From our guests",note:"Every book was chosen by someone who came, and stands under their name."},
...
function musicSpace(items){
  ...
  return `<div class="music-space"><header class="music-stage"><span class="groove" aria-hidden="true"></span><div><p>Noted listening room</p><h1>music</h1><blockquote>What the speakers are playing,<br>and what the fingers are learning.</blockquote></div><b>${playlists.reduce((n,p)=>n+p.tracks.length,0)}<small>things in rotation</small></b></header><div class="music-cabinets"><section><h2>In rotation</h2>${playlists.map(({e,tracks})=>`<article class="record-card" data-entry="${esc(e.id)}">${e.image?`<img src="${esc(e.image)}" alt="${esc(e.imageAlt||"")}">`:""}<div><span>${esc(e.title)}</span><ol>${tracks.slice(0,5).map((raw,i)=>{const t=trackParts(raw);return `<li><i>${String(i+1).padStart(2,"0")}</i>${esc(t.main)}${t.sub?`<small>${esc(t.sub)}</small>`:""}</li>`}).join("")}</ol></div></article>`).join("")}</section><aside><h2>Practice journey</h2>${practice.length?`<button class="practice-progress" data-journey="${esc(journey)}"><span>${esc(journey)}</span><b>Day ${maxDay}</b><i><em style="width:${Math.min(100,maxDay/30*100)}%"></em></i><small>${practice.length} milestones kept · open the full journey →</small></button>${practice.map(e=>`<article class="practice-slip" data-entry="${esc(e.id)}"><span>${e.day?`Day ${e.day}`:fmtDate(e.occurredAt)}</span><b>${esc(e.title.replace(/^Day\s+\d+:\s*/i,""))}</b><p>${esc(e.excerpt)}</p></article>`).join("")}`:`<p>Journey entries will collect here.</p>`}</aside></div></div>`;
}
// The wedding shelf. Same furniture as the reading room, because it is the
// same idea: a wall of covers you can read across. What changes is what a
// book is filed under. Here it is the person who chose it, so the shelf is a
// record of the guests as much as of the reading, and the giver takes the
// line where a reading status would normally sit.
function weddingSpace(items,books){
  const named=books.filter(b=>b.givenBy),givers=new Set(named.map(b=>b.givenBy));
  const notes=items.filter(e=>e.type!=="Quote"),quotes=items.filter(e=>e.type==="Quote");
  const shelf=[...books].sort((a,b)=>(a.givenBy||"~").localeCompare(b.givenBy||"~")||a.title.localeCompare(b.title));
  return `<div class="reading-space wedding-space">`+
    `<header class="reading-mast"><p>Gifts from our guests</p><h1>wedding library</h1>`+
    `<div>${books.length?`${books.length} ${books.length===1?"book":"books"} from ${givers.size} ${givers.size===1?"guest":"guests"}`:"A shelf waiting for its books"}</div></header>`+
    (quotes.length?`<section class="reading-quote-wall">${quotes.map(q=>`<blockquote data-entry="${esc(q.id)}"><span>From the commonplace book</span>&ldquo;${esc(q.title)}&rdquo;<cite>${esc(q.author||"")}</cite></blockquote>`).join("")}</section>`:"")+
    `<section class="reading-shelves"><div class="reading-label"><h2>The shelves</h2>`+
    `<span>${books.length} ${books.length===1?"volume":"volumes"}${named.length?` &middot; ${givers.size} ${givers.size===1?"giver":"givers"}`:""}</span></div>`+
    `<div class="shelf-row">${shelf.map(b=>`<article class="shelf-book" data-book="${esc(b.id)}">${b.cover?`<img data-cover="${esc(b.id)}" src="${esc(b.cover)}" alt="">`:coverPlate(b)}<div><b>${esc(b.title)}</b><small>${esc(b.author)}</small>${b.givenBy?`<span class="from">from ${esc(b.givenBy)}</span>`:`<span>${esc(b.status.replaceAll("-"," "))}</span>`}</div></article>`).join("")||`<p class="empty">No books on the shelf yet.</p>`}</div></section>`+
    (notes.length?`<section class="reading-margins"><h2>Notes</h2>${notes.map(e=>`<article data-entry="${esc(e.id)}"><time>${fmtDate(e.occurredAt||e.createdAt)}</time><b>${esc(e.title)}</b><p>${esc(e.excerpt)}</p></article>`).join("")}</section>`:"")+
  `</div>`;
}
function readingSpace(items,books){
  ...
  return `<div class="reading-space"><header class="reading-mast"><p>Private circulation library</p><h1>reading room</h1><div>${reading.length?`Currently reading · ${esc(reading[0].title)} · ${reading[0].progress}%`:"Books, marginalia and sentences worth keeping"}</div></header>${quotes.length?`<section class="reading-quote-wall">${quotes.map(q=>`<blockquote data-entry="${esc(q.id)}"><span>From the commonplace book</span>“${esc(q.title)}”<cite>${esc(q.author||"")}</cite></blockquote>`).join("")}</section>`:""}<section class="reading-shelves"><div class="reading-label"><h2>The shelves</h2><span>${books.length} volumes · ${books.filter(b=>b.status==="finished").length} finished</span></div><div class="shelf-row">${books.map(b=>`<article class="shelf-book" data-book="${esc(b.id)}">${b.cover?`<img data-cover="${esc(b.id)}" src="${esc(b.cover)}" alt="">`:coverPlate(b)}<div><b>${esc(b.title)}</b><small>${esc(b.author)}</small><span>${esc(b.status.replaceAll("-"," "))}${b.status==="reading"?` · ${b.progress}%`:""}</span></div></article>`).join("")}</div></section>${notes.length?`<section class="reading-margins"><h2>Marginalia</h2>${notes.map(e=>`<article data-entry="${esc(e.id)}"><time>${fmtDate(e.occurredAt||e.createdAt)}</time><b>${esc(e.title)}</b><p>${esc(e.excerpt)}</p></article>`).join("")}</section>`:""}</div>`;
}
// The garden is the outdoor year; the house plants are stock kept indoors.
// They share the topic, so the room shows the plants as a shelf of their own,
// with a way through to their space, rather than mixing them into the season's
// observations where they would repeat every time the list is read.
function gardeningSpace(items){
  const rows=ordered(items.filter(e=>!e.plant)),plants=ordered(items.filter(e=>e.plant));
  const lead=rows.find(e=>e.image)||plants.find(e=>e.image)||rows[0];
  return `<div class="garden-space"><header class="garden-gate">${lead?.image?`<img src="${esc(lead.image)}" alt="${esc(lead.imageAlt||"")}">`:""}<div><p>Seasonal field book · late summer</p><h1>in the garden</h1><blockquote>Jobs done, things growing,<br>weather permitting.</blockquote></div></header>${plants.length?`<section class="garden-plants"><div class="garden-plants-head"><h2>House plants</h2><button type="button" data-topic="houseplants">${plants.length} indoors, open the space &rarr;</button></div><div class="garden-plant-row">${plants.map(e=>{const due=waterDue(e.plant);return `<button type="button" class="garden-plant" data-entry="${esc(e.id)}">${e.image?`<img src="${esc(e.image)}" alt="${esc(e.imageAlt||"")}" loading="lazy">`:`<i aria-hidden="true">${icon("plant")}</i>`}<b>${esc(e.title)}</b><small>${esc(due?waterWord(due):e.plant.light||"")}</small></button>`}).join("")}</div></section>`:""}<div class="garden-bed"><section><h2>Field observations</h2><ol>${rows.map(e=>`<li data-entry="${esc(e.id)}"><time>${fmtDate(e.occurredAt||e.createdAt)}</time><span class="garden-pin"></span><div><h3>${esc(e.title)}</h3><p>${esc(e.excerpt)}</p></div>${e.image?`<img src="${esc(e.image)}" alt="">`:""}</li>`).join("")||`<p class="empty small">Nothing from the beds yet.</p>`}</ol></section><aside><span>Season tally</span><b>${rows.length}</b><p>notes from the growing year</p><i aria-hidden="true">${icon("leaf")}</i></aside></div></div>`;
}
...
// Which build is actually running. Sounds like housekeeping; it is the thing
...
// --- Quick add -------------------------------------------------------
// A single "+" reachable from anywhere, behind the PIN like the rest of
// the site. Picks a content type, fills in a few fields, submits via the
// Worker's GitHub API endpoint the same way task toggling does: it
// writes and commits the file directly, which is what triggers the
// normal rebuild. A photo, if attached, is resized and re-encoded
// client-side before upload, well under the server's 5MB cap.
...
// Title and attachments use separate areas of the composer.
  ...
  switch(type){
    ...
    case "task":
      return `<label>Title<input type="text" name="title" required autofocus></label>
        ...
        ${common}
        <label>Note (optional)<textarea name="body" rows="3"></textarea></label>`;
    case "quote":
...
function setupQuickAddFieldExtras(container){
  ...
  if(photos)fields.insertBefore(photos,tags);
...
quickAddButton.className="quick-add-fab";
quickAddButton.setAttribute("aria-label","Add to noted");
quickAddButton.innerHTML=`<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 4v16M4 12h16"/></svg>`;
...
// Local suggestions keep private writing on this device.
const QA_TAG_WORDS={family:['birthday','daughter','son','mum','dad','family','molly'],gardening:['garden','gardening','planting','lawn','grass'],houseplants:['houseplant','houseplants','monstera'],reading:['book','reading','novel'],food:['cooking','dinner','lunch','food','meal'],motoring:['car','tyre','motoring'],music:['music','guitar','piano','concert'],selfcare:['sleep','rest','meditation'],wedding:['wedding','bride','groom'],technology:['software','coding','computer']};
let qaExcludedTags=new Set(),qaBusy=false,qaDrag=null;
function suggestQuickAddTags(text,type){
  const words=new Set(text.toLowerCase().match(/[a-z0-9]+/g)||[]);
  return knownTags().filter(tag=>words.has(tag)||(QA_TAG_WORDS[tag]||[]).some(word=>words.has(word))||(type==='reading'&&tag==='reading')||(type==='recipe'&&tag==='recipes')).slice(0,5);
}
function updateQuickAddTags(){
  const form=quickAddBackdrop.querySelector('form');
  if(!form)return;
...
quickAddBackdrop.addEventListener('pointerdown',e=>{
  if(!e.target.closest('.qa-handle')||qaBusy)return;
  qaDrag={y:e.clientY};
  e.target.closest('.qa-handle').setPointerCapture(e.pointerId);
});
quickAddBackdrop.addEventListener('pointermove',e=>{
  if(!qaDrag)return;
  quickAddBackdrop.querySelector('.qa-sheet').style.transform='translateY('+Math.max(0,e.clientY-qaDrag.y)+'px)';
});
function finishQuickAddDrag(e){
  if(!qaDrag)return;
  const dismiss=e.type!=='pointercancel'&&e.clientY-qaDrag.y>80;
  qaDrag=null;
  quickAddBackdrop.querySelector('.qa-sheet').style.transform='';
  if(dismiss)closeQuickAdd();
}
quickAddBackdrop.addEventListener('pointerup',finishQuickAddDrag);
quickAddBackdrop.addEventListener('pointercancel',finishQuickAddDrag);
quickAddBackdrop.addEventListener('keydown',e=>{
  if(e.key!=='Tab')return;
  const items=[...quickAddBackdrop.querySelectorAll('button,input,textarea,select,summary')].filter(el=>!el.disabled&&el.getClientRects().length);
...
function renderQuickAddImages(){
  ...
  const add=quickAddBackdrop.querySelector(".qa-photo-add");
  const single=!quickAddBackdrop.querySelector('[name="image"]')?.multiple;
  if(add){add.hidden=!single&&quickAddImages.length>=6;add.querySelector("span").textContent=quickAddImages.length?(single?"Change cover":"Add more"):(single?"Add cover":"Add photos");}
...
function openQuickAdd(){
  if(!quickAddBackdrop.querySelector("form")){
    quickAddType=QUICK_ADD_TYPES[0].id;
    quickAddImages=[];
    qaExcludedTags=new Set();
  ...
  requestAnimationFrame(()=>quickAddBackdrop.querySelector('[name="title"]')?.focus());
}
...
async function compressImageFile(file, maxDim=2000, maxBytes=4*1024*1024){
...
quickAddBackdrop.addEventListener("click",e=>{
  if(qaBusy)return;
  const tag=e.target.closest("[data-qa-dismiss-tag]");
  if(tag){qaExcludedTags.add(tag.dataset.qaDismissTag);updateQuickAddTags();return}
  ...
  if(removeBtn){
    URL.revokeObjectURL(quickAddImages.splice(Number(removeBtn.dataset.qaRemoveImage),1)[0].url);
...
quickAddBackdrop.addEventListener("change",async e=>{
  if(e.target.matches("[data-qa-type-select]")){
    ...
    const previous=Object.fromEntries(new FormData(quickAddBackdrop.querySelector("form")));
    quickAddBackdrop.querySelector(".qa-fields").innerHTML=quickAddFields(quickAddType);
    quickAddBackdrop.querySelectorAll('input[type="text"],textarea,input[type="date"]').forEach(el=>{if(previous[el.name])el.value=previous[el.name]});
    setupQuickAddFieldExtras(quickAddBackdrop);
    renderQuickAddImages();
    updateQuickAddTags();
    return;
  ...
  if(e.target.name==="image"&&e.target.files.length){
    ...
    qaBusy=true;
    quickAddBackdrop.querySelector(".qa-submit").disabled=true;
    quickAddBackdrop.querySelector(".qa-type-select").disabled=true;
    const files=[...e.target.files];
    const isSingle=!e.target.multiple;
    if(isSingle){quickAddImages.forEach(img=>URL.revokeObjectURL(img.url));quickAddImages=[];}
    ...
    renderQuickAddImages();
    qaBusy=false;
    quickAddBackdrop.querySelector(".qa-submit").disabled=false;
    quickAddBackdrop.querySelector(".qa-type-select").disabled=false;
  }
});
...
quickAddBackdrop.addEventListener("submit",async e=>{
  ...
  try{
    ...
    delete data.image;
    quickAddBackdrop.querySelectorAll("input,textarea,select,button").forEach(el=>el.disabled=true);
    if(data.endTime&&data.endTime.trim()){
      data.startTime=`${(data.startTime||"").trim()}${data.startTime&&data.startTime.trim()?" – ":""}${data.endTime.trim()}`;
    }
    delete data.endTime;
    if(quickAddImages.length&&form.querySelector('[name="image"]')){
      const urls=[];
    ...
    qaBusy=false;
    closeQuickAdd();
    quickAddImages.forEach(img=>URL.revokeObjectURL(img.url));
    quickAddImages=[];
    quickAddBackdrop.innerHTML="";
    toast("Added, it'll appear once the site rebuilds");
  }catch(error){
...
document.addEventListener("click",async e=>{
  ...
  const jr=e.target.closest("[data-journey]");if(jr){state.returnTo=location.hash||"#library";location.hash=`journey/${encodeURIComponent(jr.dataset.journey)}`;return}
