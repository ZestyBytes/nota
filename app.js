...
function entryPage(id){
  ...
  return `<section class="entry-page entry-space-${spaceId}" style="--topic:${t.color};--space:${space.color}">
    ...
    <header class="entry-masthead">
      ...
      <div class="entry-context">${chips(e.topics)}${e.journey?(()=>{const rows=journeyEntries(e.journey),n=rows.findIndex(r=>r.id===e.id)+1;return `<p class="journey-of"><a href="#journey/${encodeURIComponent(e.journey)}">${esc(e.journey)}</a><span>${n} of ${rows.length}</span></p>`})():""}<a class="entry-space-link" href="#topics/${encodeURIComponent(spaceId)}">${esc(space.name)} space &rarr;</a></div>
    </header>
...
function topicView(id){const t=topic(id),kids=childTopics(id),items=state.data.entries.filter(e=>inTopic(e,id)),books=state.data.books.filter(b=>inTopic(b,id));let body=`<div class="entry-list">${items.map(e=>entryCard(e,"",id)).join("")||(books.length?"":`<p class="empty">Nothing in this topic yet.</p>`)}</div>`;if(t.mode==="listen"){
  ...
  if(TOPIC_ROOMS[id])return `<section class="topic-page bespoke-topic-page bespoke-${id}" style="--topic:${t.color};--soft:${t.soft}"><p class="back-link room-back"><a href="${state.returnTo||"#topics"}" data-back>Back to topics</a></p>${bespokeTopic(id,items,books,kids)}</section>`;
...
window.addEventListener("hashchange",async()=>{if(NotedBackend.configured&&!state.user&&location.hash==="#writing")state.data={...emptyArchive(),entries:await NotedBackend.loadPublished()};render()});
