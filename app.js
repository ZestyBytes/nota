...
function taskPage(){
  ...
  const progress=all.length?Math.round(finished.length/all.length*100):100;
  return `<section class="tasks-page">${pageHead("Still to do","To-do","A calm place for what is waiting—and what has been seen off.")}
    ${all.length?`<div class="task-progress"><div><span>${open.length?`${open.length} still moving`:"All clear"}</span><small>${finished.length?`${finished.length} finished`:"One thing at a time"}</small></div><i aria-label="${progress}% complete"><b style="width:${progress}%"></b></i></div>`:""}
