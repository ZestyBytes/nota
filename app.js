...
function weddingSpace(items,books){
  ...
  return `<div class="reading-space wedding-space">`+
    ...
    `<div class="shelf-row">${shelf.map(b=>`<article class="shelf-book" data-book="${esc(b.id)}">${b.cover?`<img src="${esc(b.cover)}" alt="">`:coverPlate(b)}<div><b>${esc(b.title)}</b><small>${esc(b.author)}</small>${b.givenBy?`<span class="from">from ${esc(b.givenBy)}</span>`:`<span>${esc(b.status.replaceAll("-"," "))}</span>`}</div></article>`).join("")||`<p class="empty">No books on the shelf yet.</p>`}</div></section>`+
